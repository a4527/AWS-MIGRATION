...
현재 Terraform 1차 범위에서는 Route53과 ACM HTTPS listener는 후속 작업으로 둔다. 대신 public ALB와 HTTP listener는 ECS 모듈에서 생성한다.
...
`terraform/environments/dev/providers.tf`는 AWS provider만 사용한다.

이전 구조에서는 별도 인증서 thumbprint 조회를 위해 `tls` provider가 필요했지만, 현재 ECS 구조에서는 필요하지 않다.
...
현재 ECS 모듈은 HTTP listener만 만든다. HTTPS listener와 ACM 인증서 연결은 후속 작업이다.
...
HTTPS는 후속 보강 대상이다. 운영 구조에서는 ACM 인증서를 발급하고 443 listener를 추가한 뒤, 80은 443 redirect로 바꾸는 것이 일반적이다.
...
- S3 접근은 ECS task role로 처리한다.
- Terraform 루트 모듈은 `terraform/environments/dev`이다.
- `terraform.tfvars`는 변수 값만 공급하고, 실제 리소스 구조는 `.tf` 파일과 모듈들이 결정한다.

이 구조를 기준으로 다음 단계에서는 ECR image push, ECS 배포 자동화, HTTPS, Route53, Secrets Manager, Lambda 후처리를 연결하면 된다.
